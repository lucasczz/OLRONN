...
In one of the earlier works, \citet{schaulNoMorePesky2013} propose \textit{vSGD} which uses first and second order moments of the gradients as well as local curvature information~\cite{schaulNoMorePesky2013} to estimate $\eta$.
...
However, besides \citet{schaulNoMorePesky2013} none of these optimizers target batch learning scenarios and therefore do not explicitly consider concept drift, which raises the question of whether they are suitable for stream-based learning.
...
\begin{table}[]
	\begin{tabular}{@{}lllc@{}}
		\toprule
		Optimizer & Complexity                      & Memory & Param. specific \\ \midrule
		DAdapt    & $\mathcal{O}(6D)$               & $2D$   & \xmark          \\
		DoG       & $\mathcal{O}(5D)$               & $1D$   & \xmark          \\
		Mechanic  & $\mathcal{O}(4D)$               & $1D$   & \xmark          \\
		WNGrad    & $\mathcal{O}(2D)$               & $0$    & \xmark          \\
		COCOB     & $\mathcal{O}(14D)$              & $4D$   & \cmark          \\
		vSGD      & $\mathcal{O}(21D)$\footnotemark & $4D$   & \cmark          \\\bottomrule
	\end{tabular}
	\caption{Overview of parameter-free first-order optimizers}\label{tab:param_free_optims}
\end{table}
