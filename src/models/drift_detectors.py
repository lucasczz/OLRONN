...
class PADWIN(DriftDetector):
            ...
            while bln_reduce_width:
                ...
                while (not bln_exit) and (cursor is not None):
                    for k in range(cursor.bucket_size_row - 1):
                        ...
                        abs_value = 1.0 * ((u0 / n0) - (u1 / n1))
                        if (n1 >= self.mint_min_window_length) and (
                            n0 >= self.mint_min_window_length
                        ):
                            probas.append(
                                self.__bln_cut_expression(
                                    n0, n1, u0, u1, v0, v1, abs_value, self.delta
    ...
    def __bln_cut_expression(self, n0, n1, u0, u1, v0, v1, abs_value, delta):
        ...
        p = 2 * np.log(n) / np.exp(1.5 * m * abs_value - 3 / np.sqrt(2 * m) * std)
        return 1 - p


class OneTailedADWIN(ADWIN):
    def __bln_cut_expression(self, n0, n1, u0, u1, v0, v1, abs_value, delta):
        n = self.width
        dd = np.log(2 * np.log(n) / delta)
        v = self.variance
...
class Manual:
    def __init__(self, batch_size) -> None:
        ...
        self.next_change_point = 0
        self.batch_size = batch_size
        self.drift_detected = False

    def update(self, *args):
        self._n_observed += self.batch_size
        self.drift_detected = False
        if (
            self.next_change_point < len(self.change_points)
            and self._n_observed > self.change_points[self.next_change_point]
        ):
